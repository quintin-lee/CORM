...
typedef struct corm_backend {
    ...
    corm_err_t (*exec)(struct corm *db, const char *sql, corm_value_t *params, int param_count);
    corm_err_t (*query)(struct corm *db, const char *sql, corm_value_t *params, int param_count, struct corm_result **out);
    corm_err_t (*begin)(struct corm *db);
    corm_err_t (*commit)(struct corm *db);
    corm_err_t (*rollback)(struct corm *db);
    size_t (*escape_string)(struct corm *db, char *dst, const char *src, size_t len);
