...
static corm_field_t user_fields[] = {
    CORM_FIELD(user_t, id,    CORM_INT,    CORM_FLAG_PRIMARY | CORM_FLAG_AUTOINC, NULL),
...
int main(void) {
    ...
    err = corm_exec(db, "INSERT INTO users (name, age, score) VALUES ('Alice', 30, 95.5)");
    printf("   %s\n", err == CORM_OK ? "✓ Alice inserted" : "✗ Failed");

    err = corm_exec(db, "INSERT INTO users (name, age, score) VALUES ('Bob', 25, 87.0)");
    printf("   %s\n", err == CORM_OK ? "✓ Bob inserted" : "✗ Failed");

    err = corm_exec(db, "INSERT INTO users (name, age, score) VALUES ('Charlie', 35, 92.3)");
    ...
    if (err == CORM_OK && res) {
        ...
        while (corm_result_next(res)) {
            ...
            printf("   %d. [%d] %s — Age: %d, Score: %.1f\n",
                   row_num++, id, name ? name : "?", age, score);
        }
    ...
    err = corm_begin(db);
    if (err == CORM_OK) printf("   ✓ BEGIN\n");

    err = corm_exec(db, "INSERT INTO users (name, age, score) VALUES ('Dave', 28, 78.5)");
    if (err == CORM_OK) printf("   ✓ Dave inserted\n");

    err = corm_commit(db);
    if (err == CORM_OK) printf("   ✓ COMMIT\n");
    printf("\n");
