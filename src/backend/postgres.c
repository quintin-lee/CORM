...
static corm_err_t pg_open(corm_t *db, const char *dsn) {
    ...
    p += snprintf(p, sizeof(conninfo) - (size_t)(p - conninfo),
...
static corm_err_t pg_ping(corm_t *db) {
    ...
    if (PQstatus(handle) != CONNECTION_OK) return CORM_ERR_BACKEND;
    return CORM_OK;
}

static corm_err_t pg_exec(corm_t *db, const char *sql,
                          corm_value_t *params, int param_count) {
    PGconn *handle = (PGconn *)db->conn;

    if (param_count > 0) {
        /* Parameterized execution */
        const char * const *param_values = (const char * const *)
...
static corm_err_t pg_query(corm_t *db, const char *sql,
                           corm_value_t *params, int param_count,
                           corm_result_t **out) {
    PGconn *handle = (PGconn *)db->conn;
    PGresult *pgres;
    ...
    if (param_count > 0) {
        ...
        pgres = PQexecParams(handle, sql, param_count, NULL,
                             param_values, param_lengths, param_formats, 0);
    ...
    for (int r = 0; r < row_count; r++) {
        for (int i = 0; i < col_count; i++) {
            ...
            switch (v->type) {
                ...
                case CORM_BLOB: {
                    size_t len;
                    unsigned char *blob = PQunescapeBytea(
                        (unsigned char *)PQgetvalue(pgres, r, i), &len);
...
static corm_err_t pg_begin(corm_t *db) {
    return pg_exec(db, "BEGIN", NULL, 0);
}
...
__attribute__((constructor))
static void pg_stub_register(void) {
