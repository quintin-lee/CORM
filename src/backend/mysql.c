...
static corm_err_t mysql_open(corm_t *db, const char *dsn) {
    ...
    unsigned int conn_timeout = db->config.timeout_ms > 0 ?
                                (unsigned int)(db->config.timeout_ms / 1000) : 30;
    mysql_options(handle, MYSQL_OPT_CONNECT_TIMEOUT, &conn_timeout);

    if (!mysql_real_connect(handle, host, user, pass,
                            dbname[0] ? dbname : NULL, (unsigned int)port,
                            NULL, 0)) {
...
static corm_err_t mysql_exec(corm_t *db, const char *sql,
...
static void mysql_register(void) {
...
__attribute__((constructor))
static void mysql_stub_register(void) {
