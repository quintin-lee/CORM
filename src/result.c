...
corm_result_t *corm_result_new(int column_count, int row_count) {
  ...
  r->column_types = (corm_field_type_t *)calloc((size_t)column_count,
                                                sizeof(corm_field_type_t));
  if (!r->column_types) {
    free(r->column_names);
    free(r);
    return NULL;
  ...
  if (row_count > 0) {
    r->rows =
        (corm_value_t **)calloc((size_t)row_count, sizeof(corm_value_t *));
    if (!r->rows) {
      free(r->column_types);
      free(r->column_names);
      free(r);
    ...
    for (int i = 0; i < row_count; i++) {
      r->rows[i] =
          (corm_value_t *)calloc((size_t)column_count, sizeof(corm_value_t));
      if (!r->rows[i]) {
        for (int j = 0; j < i; j++)
          free(r->rows[j]);
        free(r->rows);
...
void corm_result_reset(corm_result_t *r) { r->current_row = -1; }
