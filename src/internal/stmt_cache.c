...
corm_err_t corm_stmt_cache_create(size_t capacity, corm_stmt_cache_t **out_cache) {
    if (!out_cache || capacity == 0) return CORM_ERR_NULL;
    corm_stmt_cache_t *cache = calloc(1, sizeof(corm_stmt_cache_t));
    if (!cache) return CORM_ERR_NOMEM;
...
corm_err_t corm_stmt_cache_put(corm_stmt_cache_t *cache, const char *sql, void *stmt) {
    if (!cache || !sql || !stmt) return CORM_ERR_NULL;

    // Check if exists
    void *existing = corm_stmt_cache_get(cache, sql);
    if (existing) return CORM_OK;
