...
static inline corm_err_t corm_strbuf_appendn(corm_strbuf_t *sb, const char *s, size_t n) {
...
static inline corm_err_t corm_strbuf_appendf(corm_strbuf_t *sb, const char *fmt, ...) {
    ...
    va_end(ap);
    if (needed < 0) return CORM_ERR_GENERIC;
