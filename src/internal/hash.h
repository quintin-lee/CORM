...
static inline corm_err_t corm_hash_init(corm_hash_t *ht, size_t initial_size) {
  if (initial_size == 0)
    initial_size = 32;
  ht->buckets =
      (corm_hash_entry_t **)calloc(initial_size, sizeof(corm_hash_entry_t *));
  if (!ht->buckets)
...
static inline corm_err_t corm_hash_insert(corm_hash_t *ht, const char *key,
                                          void *value) {
  size_t idx = corm_hash_str(key, ht->size);
  corm_hash_entry_t *entry =
      (corm_hash_entry_t *)malloc(sizeof(corm_hash_entry_t));
  if (!entry)
    return CORM_ERR_NOMEM;
