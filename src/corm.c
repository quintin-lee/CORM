...
static corm_backend_type_t parse_backend(const char *dsn, const char **out_start) {
...
corm_err_t corm_open_with_config(const char *dsn, corm_config_t config, corm_t **out) {
    if (!dsn || !out) return CORM_ERR_NULL;

    corm_t *db = (corm_t *)calloc(1, sizeof(corm_t));
    if (!db) return CORM_ERR_NOMEM;
...
corm_err_t corm_ping(corm_t *db) {
    if (!db || !db->backend) return CORM_ERR_NULL;
...
const char *corm_err_string(corm_err_t err) {
    switch (err) {
        case CORM_OK:            return "OK";
        case CORM_ERR_GENERIC:   return "Generic error";
        case CORM_ERR_NOMEM:     return "Out of memory";
        case CORM_ERR_NOTFOUND:  return "Not found";
        case CORM_ERR_DUP:       return "Duplicate entry";
        case CORM_ERR_BACKEND:   return "Backend error";
        case CORM_ERR_TYPE:      return "Type mismatch";
        case CORM_ERR_NULL:      return "Null pointer";
        case CORM_ERR_BOUNDS:    return "Out of bounds";
        case CORM_ERR_MISMATCH:  return "Mismatch";
        default:                 return "Unknown error";
    }
}

corm_err_t corm_last_error(corm_t *db, char *buf, size_t bufsz) {
    if (!db || !buf) return CORM_ERR_NULL;
    snprintf(buf, bufsz, "%s: %s (SQL: %s)",
             corm_err_string(db->last_err),
