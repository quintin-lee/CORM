...
static corm_field_t rec_fields[] = {
    CORM_FIELD(HookRecord, id, CORM_INT, CORM_FLAG_PRIMARY | CORM_FLAG_AUTOINC,
               NULL),
    CORM_FIELD(HookRecord, name, CORM_STRING, 0, NULL),
};

static corm_err_t on_before_update(corm_t *db, void *record) {
  (void)db;
  (void)record;
  before_update_called = true;
  return CORM_OK;
}
static corm_err_t on_after_update(corm_t *db, void *record) {
  (void)db;
  (void)record;
  after_update_called = true;
  return CORM_OK;
}
static corm_err_t on_before_delete(corm_t *db, void *record) {
  (void)db;
  (void)record;
  before_delete_called = true;
  return CORM_OK;
}
static corm_err_t on_after_delete(corm_t *db, void *record) {
  (void)db;
  (void)record;
  after_delete_called = true;
  return CORM_OK;
}
static corm_err_t on_after_find(corm_t *db, void *record) {
  (void)db;
  (void)record;
