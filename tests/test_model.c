...
static corm_field_t test_user_fields[] = {
    ...
    { .name = "age",    .type = CORM_INT,    .offset = offsetof(test_user_t, age),   .size = sizeof(int),      .flags = 0, .default_value = "0" },
    { .name = "score",  .type = CORM_FLOAT,  .offset = offsetof(test_user_t, score), .size = sizeof(float) },
    { .name = "active", .type = CORM_BOOL,   .offset = offsetof(test_user_t, active),.size = sizeof(bool) },
};
...
static void test_field_get_set(void) {
    ...
    test_user_t user = { .id = 42, .name = "test", .age = 25, .score = 3.5f, .active = true };
