...
static void test_sqlite_types(void) {
    ...
    err = corm_exec(db, "INSERT INTO type_test (id, ival, bval, tval, blobval, rval) "
    ...
    err = corm_exec(db, "INSERT INTO type_test (id, ival) VALUES (3, 3000000000)");
    assert(err == CORM_OK);
    PASS();

    TEST("INSERT BLOB with binary data");
    err = corm_exec(db, "INSERT INTO type_test (id, ival, blobval) VALUES (4, 99, x'a1b2c3d4e5f6')");
    assert(err == CORM_OK);
    PASS();
